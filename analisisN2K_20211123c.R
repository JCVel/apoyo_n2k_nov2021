...
# Cambiar el nombre del campo  SITE_PG_NAME a NORMA, para que encaje con los datos 
# que nos han pasado de IP_ENP
names(IP_RN_paraIP)[names(IP_RN_paraIP) == "SITE_PG_NAME"] <- "NORMA_RN"

IP_RN_paraIP <- subset(IP_RN_paraIP, select = -c(SITE_PG_DATE))
IP_RN_paraIP$TIPO <- "PG RN2000"
# Unir a IP_nbr
...
IP_nbr <- subset(IP_nbr, select = -c(NORMA_RN, ANIO_NORMA_RN))
