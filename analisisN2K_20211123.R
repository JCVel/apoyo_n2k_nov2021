...
#########################
# Instrumentos planificación ENPs (IPENP)
#########################
# IPENP de los ENP (proporcionados por el dpto de SIG de TTEC)
path_nbr_IPENP <- file.path(pathEntrada, "ENP_planes.xlsx")
nbr_IPENP_ori <- read_excel(path_nbr_IPENP)
nbr_IPENP <- nbr_IPENP_ori

# Comprobar si hay duplicados --> Sí, hay duplicados
print(paste0("Nº de registros de los datos del dpto SIG TTEC: ", nrow(nbr_IPENP)))
print(paste0("Nº de registros de los datos del dpto SIG TTEC: ", nrow(unique(nbr_IPENP))))
# Eliminar duplicados
nbr_IPENP <- unique(nbr_IPENP)
# Comprobar que los SITE_CODE de nbr_IPENP coinciden con SITE_CODE de EPAct
# all(nbr_IPENP$SITE_CODE %in% EPAct$SITE_CODE) # Sí. Sale TRUE

# Añadir id y registro para los espacios que no tienen IPENP
nbr_IPENP$id <- paste0("IPENP_", seq(1, nrow(nbr_IPENP)))
nbr_IPENP <- nbr_IPENP[ , c("id", "ANIO_NORMA", "NORMA", "TIPO", "URL", "SITE_CODE")]
df <- data.frame(
  id = "IPENP_0",
  ANIO_NORMA = 0,
  NORMA = "No hay un PG registrado",
  TIPO = "",
  URL = "",
  SITE_CODE = ""
)
nbr_IPENP <- rbind(nbr_IPENP, df)
rm(df)

# Sacar la tabla intermedia de la relacion 1:N
nbr_IPENP_nbr <- nbr_IPENP[ , c("id", "SITE_CODE")]
nbr_IPENP_nbr <- nbr_IPENP_nbr[nbr_IPENP_nbr$id != "IPENP_0", ]

# Ocurren 2 cosas: 
# CASO 1: hay ENPs que están en DD_nbr pero que no están en la tabla que nos han pasado
# Se sacan de DD_nbr y se meten en nbr_IPENP_nbr
nbr_IPENP_nbrVect <- unique(nbr_IPENP_nbr$SITE_CODE)

nbr_IPENP_nbr2 <- data.frame(
  SITE_CODE = DD_nbr$SITE_CODE)

nbr_IPENP_nbr2$IPENP <- ifelse(nbr_IPENP_nbr2$SITE_CODE %in% nbr_IPENP_nbrVect, "Tiene IPENP", "No tiene IPENP" )
data.frame(table(nbr_IPENP_nbr2$IPENP))
# Seleccionar los 39 que no tienen IPENP
nbr_IPENP_nbr3 <- nbr_IPENP_nbr2[nbr_IPENP_nbr2$IPENP == "No tiene IPENP", ]
nbr_IPENP_nbr3$id <- "IPENP_0"
nbr_IPENP_nbr3 <- nbr_IPENP_nbr3[ , c("SITE_CODE", "id")]
nbr_IPENP_nbr <- rbind(nbr_IPENP_nbr, nbr_IPENP_nbr3)
rm(nbr_IPENP_nbr2, nbr_IPENP_nbr3, nbr_IPENP_nbrVect)

# CASO 2: hay ENPs que están en la tabla nbr_IPENP_nbr pero no en DD_nbr (espacios vecinos que no están 
#   incluidos porque no cortan con RN2000). Se quitan de nbr_IPENP_nbr
# Se saca vector con los espacios de DD_nbr
DD_nbr_vect <- DD_nbr$SITE_CODE
nbr_IPENP_nbr$Quitar <- ifelse(nbr_IPENP_nbr$SITE_CODE %in% DD_nbr_vect, "Dejar", "Quitar" )
data.frame(table(nbr_IPENP_nbr$Quitar))
nbr_IPENP_nbr <- nbr_IPENP_nbr[nbr_IPENP_nbr$Quitar != "Quitar", ]
nbr_IPENP_nbr <- subset(nbr_IPENP_nbr, select = - c(Quitar))
################################################################################
###___ ExportaciOn de datos
################################################################################
# Crear lista con los nombres de los objetos y con los objetos (modificar segUn necesidades)
var_list <- list(
  sheets = c("RNProxiAct", "DD_src", "DD_nbr", "nbr_IPENP", "nbr_IPENP_nbr"),
  objects = list(RNProxiAct, DD_src, DD_nbr, nbr_IPENP, nbr_IPENP_nbr)
)
