...
RNProxiComplDupli <- RNProxiCompl[ , c("src_SITE_CODE", "nbr_SITE_CODE")]
RNProxiComplDupli <- unique(RNProxiComplDupli) # ConclusiOn: No hay duplicados en dichas combinaciones
# pues no cambia el numero de registros
rm(RNProxiComplDupli)
...
# Eliminar los duplicados al combinar src_SITE_CODE2 y nbr_SITE_CODE2 (es decir, 
# los tipo C que tienen los datos por duplicado)
RNProxiComplSinDupl <- distinct(RNProxiComplSinDupl, src_SITE_CODE2, nbr_SITE_CODE2,
                                .keep_all = TRUE)
...
# En la capa de polígonos del cruce, cambiar el nombre del campo del área de de solape
# y eliminar el de Shape_Length
names(cruceAPIsPolySinDupl)[names(cruceAPIsPolySinDupl) == "Shape_Area"] <- "AREA"
...
names(srcArea)[names(srcArea) == "Shape_Area"] <- "Src_Area"
cruceAPIsPolySinDupl <- merge(cruceAPIsPolySinDupl, srcArea[ , c("src_SITE_CODE", "Src_Area")], 
...
names(NbrArea)[names(NbrArea) == "Shape_Area"] <- "Nbr_Area"
cruceAPIsPolySinDupl <- merge(cruceAPIsPolySinDupl, NbrArea[ , c("nbr_SITE_CODE", "Nbr_Area")], 
...
var_list <- list(
  sheets = c("RNProxiComplSinDupl2"),
  objects = list(RNProxiComplSinDupl2)
)
