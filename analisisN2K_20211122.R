...
#########################
# IP de los ENP (proporcionados por el dpto de SIG de TTEC)
path_nbr_IP <- file.path(pathEntrada, "ENP_planes.xlsx")
nbr_IP_ori <- read_excel(path_nbr_IP)
nbr_IP <- nbr_IP_ori

# Comprobar si hay duplicados --> Sí, hay duplicados
print(paste0("Nº de registros de los datos del dpto SIG TTEC: ", nrow(nbr_IP)))
print(paste0("Nº de registros de los datos del dpto SIG TTEC: ", nrow(unique(nbr_IP))))
# Eliminar duplicados
nbr_IP <- unique(nbr_IP)
# Comprobar que los SITE_CODE de nbr_IP coinciden con SITE_CODE de EPAct
# all(nbr_IP$SITE_CODE %in% EPAct$SITE_CODE) # Sí. Sale TRUE
# Limpiar la tabla y añadir registro para los espacios que no tienen IP
nbr_IP$id <- paste0("IPEP_", seq(1, nrow(nbr_IP)))
nbr_IP <- nbr_IP[ , c("id", "ANIO_NORMA", "NORMA", "TIPO", "URL", "SITE_CODE")]
df <- data.frame(
  id = "IPEP_0",
  ANIO_NORMA = 0,
...
nbr_IP <- rbind(nbr_IP, df)
rm(df)

# Sacar la tabla intermedia de la relacion 1:N
nbr_IP_nbr <- nbr_IP[ , c("id", "SITE_CODE")]
nbr_IP_nbr <- nbr_IP_nbr[nbr_IP_nbr$id != "IPEP_0", ]
nbr_IP_nbrVect <- unique(nbr_IP_nbr$SITE_CODE)
# MArcar que ENPs no están en la tabla anterior
nbr_IP_nbr2 <- data.frame(
  SITE_CODE = DD_nbr[grepl("CDDA", DD_nbr$SITE_CODE), c("SITE_CODE") ]) # AQUIIIIII hayq ue quitar esto y dejar todos los registros para que enlace bien en access

nbr_IP_nbr2$IP <- ifelse(nbr_IP_nbr2$SITE_CODE %in% nbr_IP_nbrVect, "Tiene IP", "No tiene IP" )
data.frame(table(nbr_IP_nbr2$IP))
# Seleccionar los 39 que no tienen IP
nbr_IP_nbr3 <- nbr_IP_nbr2[nbr_IP_nbr2$IP == "No tiene IP", ]
nbr_IP_nbr3$id <- "IPEP_0"
nbr_IP_nbr3 <- nbr_IP_nbr3[ , c("SITE_CODE", "id")]
nbr_IP_nbr <- rbind(nbr_IP_nbr, nbr_IP_nbr3)
rm(nbr_IP_nbr2, nbr_IP_nbr3)

# Planes de RN2000
# src_planes <-
# AQUI, hay que hcer que nbr_planes y nbr_planes_nbr 
...
var_list <- list(
  sheets = c("RNProxiAct", "DD_src", "DD_nbr", "nbr_IP", "nbr_IP_nbr"),
  objects = list(RNProxiAct, DD_src, DD_nbr, nbr_IP, nbr_IP_nbr)
)
