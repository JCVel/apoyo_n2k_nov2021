...
# OJO: el campo SITE_PG_NAME está limitado a 255 bytes.

#########################
# Instrumentos planificación ENPs (IPENP)
#########################
# IPENP de los ENP (proporcionados por el dpto de SIG de TTEC)
path_IP <- file.path(pathEntrada, "ENP_planes.xlsx")
IP_ori <- read_excel(path_IP)
IP <- IP_ori

# Comprobar si hay duplicados --> Sí, hay duplicados
print(paste0("Nº de registros de los datos del dpto SIG TTEC: ", nrow(IP)))
print(paste0("Nº de registros de los datos del dpto SIG TTEC: ", nrow(unique(IP))))
# Eliminar duplicados
IP <- unique(IP)
# Comprobar que los SITE_CODE de IP coinciden con SITE_CODE de EPAct
# all(IP$SITE_CODE %in% EPAct$SITE_CODE) # Sí. Sale TRUE

# Añadir id y registro para los espacios que no tienen IPENP
IP$id <- paste0("IPENP_", seq(1, nrow(IP)))
IP <- IP[ , c("id", "ANIO_NORMA", "NORMA", "TIPO", "URL", "SITE_CODE")]
df <- data.frame(
  id = "IPENP_0",
  ANIO_NORMA = 0,
...
# Meter los pg de RN2000 para los sitios que haya en 
...
IP_nbr <- IP[ , c("id", "SITE_CODE")]
IP_nbr <- IP_nbr[IP_nbr$id != "IPENP_0", ]

# Ocurren 2 cosas: 
# CASO 1: hay ENPs que están en DD_nbr pero que no están en la tabla que nos han pasado
# Se sacan de DD_nbr y se meten en IP_nbr
IP_nbrVect <- unique(IP_nbr$SITE_CODE)

IP_nbr2 <- data.frame(
  SITE_CODE = DD_nbr$SITE_CODE)

IP_nbr2$IPENP <- ifelse(IP_nbr2$SITE_CODE %in% IP_nbrVect, "Tiene IPENP", "No tiene IPENP" )
data.frame(table(IP_nbr2$IPENP))
# Seleccionar los 39 que no tienen IPENP
IP_nbr3 <- IP_nbr2[IP_nbr2$IPENP == "No tiene IPENP", ]
IP_nbr3$id <- "IPENP_0"
IP_nbr3 <- IP_nbr3[ , c("SITE_CODE", "id")]
IP_nbr <- rbind(IP_nbr, IP_nbr3)
rm(IP_nbr2, IP_nbr3, IP_nbrVect)

# CASO 2: hay ENPs que están en la tabla IP_nbr pero no en DD_nbr (espacios vecinos que no están 
#   incluidos porque no cortan con RN2000). Se quitan de IP_nbr
# Se saca vector con los espacios de DD_nbr
DD_nbr_vect <- DD_nbr$SITE_CODE
IP_nbr$Quitar <- ifelse(IP_nbr$SITE_CODE %in% DD_nbr_vect, "Dejar", "Quitar")
data.frame(table(IP_nbr$Quitar))
IP_nbr <- IP_nbr[IP_nbr$Quitar != "Quitar", ]
IP_nbr <- subset(IP_nbr, select = - c(Quitar))
...
var_list <- list(
  sheets = c("RNProxiAct", "DD_src", "DD_nbr", "IP_ENP", "IP_ENP_nbr"),
  objects = list(RNProxiAct, DD_src, DD_nbr, IP_ENP, IP_ENP_nbr)
)
